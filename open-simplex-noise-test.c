...
int main(__attribute__((unused)) int argc, __attribute__((unused)) char *argv[])
{
	...
	for (y = 0; y < HEIGHT; y++) {
		for (x = 0; x < WIDTH; x++) {
			...
			v0 = open_simplex_noise4(ctx, (double) x / FEATURE_SIZE / 4, (double) y / FEATURE_SIZE / 4, 0.0, 0.0);
			v1 = open_simplex_noise4(ctx, (double) x / FEATURE_SIZE / 2, (double) y / FEATURE_SIZE / 2, 0.0, 0.0);
			v2 = open_simplex_noise4(ctx, (double) x / FEATURE_SIZE / 1, (double) y / FEATURE_SIZE / 1, 0.0, 0.0);
